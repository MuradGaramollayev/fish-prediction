...
def calculate_fish_score(data: dict):
    ...
    direction_score = 0.5 + 0.5 * np.cos(np.radians(data["wind_direction"]))
    seasonal_score = 0.5 + 0.5 * np.sin(datetime.utcnow().timetuple().tm_yday / 365 * 2 * np.pi)
...
def predict_fish(req: PredictRequest):
    ...
    score = calculate_fish_score(data)
    if score >= 70:
        level = "High"
    elif score >= 40:
        level = "Medium"
